...
export default class AppUpdater extends EventEmitter {
  ...
  async downloadUpdate(update: Release) {
    ...
    const filename = `${path.basename(asset.name)}-upgrade${path.extname(asset.name)}`

    const downloader = new EasyDl(asset.browser_download_url, path.join(directory, filename), {
    ...
    downloader.wait().then(async (state) => {
      if (!state) return
      this.emit('updater-downloaded', downloader.savedFilePath || path.join(directory, filename))
    })
  ...
  async quitAndInstall(setupPath: string) {
    if (!fs.existsSync(setupPath)) return console.warn("file doesn't exist", setupPath)
