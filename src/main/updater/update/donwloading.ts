import { ProgressBarState } from '@app/renderer/shared/update'
...
export class DownloaderWindow extends DownloadingWindow {
  ...
  public static fromWebContents(webContents: Electron.WebContents): DownloaderWindow | null {
  ...
  onGetChunk(size: number) {
    ...
    const speed = Math.round(this.speedTransfer / Math.ceil((Date.now() - this.startTime) / 1000))
    this.onSpeed(speed)
