...
interface ApiRender extends IpcRenderer {
  on<Key extends keyof ApiMain.Render.OnMethods>(
    channel: Key,
    listener: (event: IpcRendererEvent, ...args: Parameters<ApiMain.Render.OnMethods[Key]>) => any
  ): () => void

  once<Key extends keyof ApiMain.Render.OnceMethods>(
    channel: Key,
    listener: (event: IpcRendererEvent, ...args: Parameters<ApiMain.Render.OnceMethods[Key]>) => any
  ): () => void
  ...
  invoke<Key extends keyof (ApiMain.HandleMethods & ApiMain.HandleOnceMethods)>(
    channel: Key,
    ...args: Parameters<(ApiMain.HandleMethods & ApiMain.HandleOnceMethods)[Key]>
  ): Promise<Awaited<ReturnType<(ApiMain.HandleMethods & ApiMain.HandleOnceMethods)[Key]>>>
}
